  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      child: PaginatedDataTable(
        header: const Text ('적금 목록 조회'),
        rowsPerPage: _rowsPerPage,
        availableRowsPerPage: const <int>[3, 10, 20],
...
class AccDataSource extends DataTableSource {
  ...
  DataRow? getRow(int index) {
    ...
    final Account acc = _accounts[index];
    return DataRow.byIndex(
      index: index,
      selected: acc.selected,
      onSelectChanged: (bool? value) {
        if (value == null) return;
        if (acc.selected != value) {
      ...
      cells: <DataCell>[
        DataCell(Text(acc.bank)),
        DataCell(Text(acc.accNum.toString())),
        DataCell(Text(acc.maxAmount.toString())),
